...
class EPCParser(DocumentParser):
    ...
    def parse(self, file_path: Path) -> ParseResult:
        """Parse an EPC (PDF or image) and extract key fields."""
        result = ParseResult()

        # Handle image files - require manual date entry
        if self.is_image_file(file_path):
            result.warnings.append("Image file uploaded - please enter dates and rating manually.")
            ...
            result.confidence_scores['rating'] = 'NOT_FOUND'
            return result
