...
@app.route("/drive")
def drive_settings():
    """Google Drive settings and sync page."""
...
def oauth_google():
    ...
    if not drive.is_configured:
        flash("Google API not configured. Add GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET to .env", "error")
...
def oauth_callback():
    ...
    if drive.handle_callback(authorization_response, redirect_uri):
        flash("Successfully connected to Google Drive!", "success")
    else:
        flash("Failed to connect to Google Drive", "error")
...
def drive_scan():
    """Scan Google Drive for property folders."""
    from services.google_drive import GoogleDriveService
    drive = GoogleDriveService()
    ...
    # Scan Drive for property folders
    folders = drive.scan_property_folders()

    # Match folders to properties
    matched = []
    for folder in folders:
        property_id = drive.match_folder_to_property(folder['folder_name'], properties)
        prop = None
        if property_id:
            prop = db.get_property(property_id)
...
def drive_sync():
    """Sync selected documents from Google Drive."""
    from services.google_drive import GoogleDriveService
    drive = GoogleDriveService()
    ...
    # Get selected files to sync
    # Format: sync_<folder_id>_<file_id>=<property_id>
    synced_count = 0
    errors = []

    for key, property_id in request.form.items():
        if not key.startswith('sync_') or not property_id:
            continue
        ...
        file_id = parts[2]
        file_name = request.form.get(f'name_{file_id}', 'document.pdf')
        doc_type = request.form.get(f'type_{file_id}', '')

        if not property_id or property_id == 'skip':
            continue
        ...
        filepath = UPLOAD_FOLDER / f"drive_{property_id}_{filename}"

        if not drive.download_file(file_id, filepath):
            errors.append(f"Failed to download: {file_name}")
            continue

        # Process based on document type
        try:
            if doc_type == 'tenancy':
                # Parse tenancy agreement
                parser = TenancyParser()
                result = parser.parse(filepath)

                # Check if property already has a tenancy
                existing = db.list_tenancies_for_property(property_id, active_only=True)
                if not existing:
                    # Create tenancy from extracted data
                    tenancy = Tenancy(
                        property_id=property_id,
                        tenant_names=result.extracted_fields.get('tenant_names', 'Unknown'),
                        tenancy_start_date=result.extracted_fields.get('tenancy_start_date'),
                        fixed_term_end_date=result.extracted_fields.get('fixed_term_end_date'),
                        rent_amount=result.extracted_fields.get('rent_amount') or Decimal('0'),
                        rent_frequency=RentFrequency(result.extracted_fields.get('rent_frequency', 'monthly')),
                        deposit_amount=result.extracted_fields.get('deposit_amount') or Decimal('0'),
                        document_path=str(filepath),
                    )
                    tenancy_id = db.create_tenancy(tenancy)
                    ...
                    tenancy = db.get_tenancy(tenancy_id)
                    tl = TimelineGenerator(db)
                    tl.generate_for_tenancy(tenancy)

                synced_count += 1

            elif doc_type in ('gas_safety', 'eicr', 'epc'):
                # Parse certificate
                parsers = {
                    'gas_safety': GasSafetyParser(),
                    'eicr': EICRParser(),
                ...
                # Build notes
                notes_parts = []
                if doc_type == 'epc' and result.extracted_fields.get('rating'):
                    notes_parts.append(f"Rating: {result.extracted_fields['rating']}")
                if doc_type == 'gas_safety' and result.extracted_fields.get('gas_safe_number'):
                    notes_parts.append(f"Gas Safe: {result.extracted_fields['gas_safe_number']}")

                cert = Certificate(
                    property_id=property_id,
                    certificate_type=cert_type_map[doc_type],
                    issue_date=result.extracted_fields.get('issue_date'),
                    expiry_date=result.extracted_fields.get('expiry_date'),
                    document_path=str(filepath),
                    notes="; ".join(notes_parts) if notes_parts else "",
                ...
                errors.append(f"Unknown document type for: {file_name}")

        except Exception as e:
            errors.append(f"Error processing {file_name}: {e}")

    if synced_count > 0:
        flash(f"Successfully synced {synced_count} document(s) from Google Drive!", "success")
